...
%EXP_STAGE4A7_2_R2_1_2_STATISTICS_ARCHIVE Reanalyse v3 with cluster CI.
    ...
    manifest=canonical_rows(root,source_root,formal,paired,s.ids);write_rows(fullfile(out,'canonical_manifest.csv'),manifest);write_rows(fullfile(out,'source_result_status.csv'),source_status(root));
...
function rows=source_status(root)
    ...
    for k=1:numel(paths),rows(end+1)=struct('relative_path',paths{k},'result_label',labels{k},'archive_status','read_only_preserved','reanalysis_role',use{k},'exists',exist(fullfile(root,paths{k}),'dir')==7);end %#ok<AGROW>
end
