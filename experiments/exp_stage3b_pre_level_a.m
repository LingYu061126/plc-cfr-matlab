...
%EXP_STAGE3B_PRE_LEVEL_A Run isolated analytic NB/BB extrapolation diagnostic.
    ...
    for li=1:numel(cfg.levels)
        for bi={'BB','NB'}
            band=stage3b_pre_band_selector(bi{1},cfg.levels{li});fair=stage3b_pre_fair_sampling(band,cfg.total_injected_energy,cfg.target_observation_time_s);
            for ci=1:numel(cfg.theta_cases)
                ...
                for si=1:numel(cfg.snr_db)
                    ...
                    r=r+1; candidate_row=summarize(band,fair,cfg,theta,cfg.snr_db(si),out);
                    if r==1
                        rows=candidate_row;
                    else
...
function row=summarize(band,fair,cfg,theta,snr,out)
    ...
    row=struct('analysis_kind',cfg.analysis_kind,'level',band.level,'band',band.name, ...
        ...
        'total_injected_energy',fair.total_injected_energy,'repetitions',fair.repetitions, ...
        'observation_time_s',fair.achieved_observation_time_s,'strict_accuracy',m.strict_topology_accuracy, ...
        ...
        'cfr_sampling_nmse',out.cfr_sampling_nmse);
end
...
function make_figure(T,dir_out)
    ...
    bar(categorical(strcat(string(z.level),' ',string(z.band))),[z.strict_accuracy,z.equivalence_class_accuracy]);grid on;ylim([0 1]);ylabel('Accuracy');legend('strict','equivalence class','Location','best');title('Stage3B-pre analytic extrapolation diagnostic: ideal CFR');
