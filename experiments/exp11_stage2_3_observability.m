...
function write_outputs(cfg,sc,pairwise,summary,confusion,config,records,audits,candidates,mode,elapsed)
    ...
    writetable(struct2table(pairwise),fullfile(cfg.results_data,[prefix '_pairwise_distance.csv']));writetable(struct2table(summary),fullfile(cfg.results_data,[prefix '_summary.csv']));writetable(struct2table(confusion),fullfile(cfg.results_data,[prefix '_confusion.csv']));writetable(struct2table(config),fullfile(cfg.results_data,[prefix '_config.csv']));writetable(struct2table(records),fullfile(cfg.results_data,[prefix '_trials.csv']));
...
function r=empty_pair(),r=struct('measurement_kind','','topology_i','','topology_j','','complex_distance',NaN,'same_equivalence_class',false,'class_i','','class_j','','view_count',0,'tie_tolerance',NaN);end
