...
%   rejection statistic on independent in-domain scenarios.
    ...
    identity.experiment_hash=experiment_hash;identity.end_time_utc=utc_now();identity.runtime_s=toc(t0);identity.selected_method=selected;identity.domain_calibration_hash=domain_model.calibration_hash;identity.frozen_topology_experiment_hash=ids.experiment_hash;identity.frozen_topology_calibration_hash=frozen_model.calibration_hash;
    write_rows(fullfile(out,'frozen_config.csv'),stamp_rows(config_row(sc,ids,frozen_model,experiment_hash,domain_model),identity));
    ...
    write_rows(fullfile(out,'scenario_manifest.csv'),stamp_rows([manifest_rows(dev); manifest_rows(cal); manifest_rows(pilot)],identity));
    write_rows(fullfile(out,'development_method_selection.csv'),stamp_rows(method_rows,identity));
    write_rows(fullfile(out,'parameter_domain_calibration.csv'),stamp_rows(calibration_rows(cal,domain_model,experiment_hash),identity));
    write_rows(fullfile(out,'paired_domain_validation.csv'),stamp_rows(decision_rows(pilot),identity));
    write_rows(fullfile(out,'domain_validation_metrics.csv'),stamp_rows(metrics,identity));
    write_rows(fullfile(out,'lower_upper_ood_summary.csv'),stamp_rows(metrics(contains({metrics.category},'lower')|contains({metrics.category},'upper')),identity));
    write_rows(fullfile(out,'nonunique_benchmark_ledger.csv'),stamp_rows(non_rows,identity));
    write_rows(fullfile(out,'nonunique_equivalence_audit.csv'),stamp_rows(non_audit,identity));
    write_rows(fullfile(out,'false_unique_summary.csv'),stamp_rows(non_metrics,identity));
    write_rows(fullfile(out,'selection_rule_sensitivity.csv'),stamp_rows(selection_sensitivity(method_rows,pilot,sc),identity));
    runtime=struct('stage_name',sc.stage_name,'mode',mode,'status',[mode '_completed'],'candidate_count',numel(candidates),'development_count',numel(dev),'calibration_count',numel(cal),'pilot_count',numel(pilot),'nonunique_row_count',numel(non_rows),'worker_count',1,'use_parallel',false,'runtime_s',identity.runtime_s,'experiment_hash',experiment_hash,'frozen_candidate_experiment_hash',ids.experiment_hash,'domain_calibration_hash',domain_model.calibration_hash,'final_reserved_status',sc.final_reserved.status,'stage4b_started',false);
    write_rows(fullfile(out,'runtime_summary.csv'),stamp_rows(runtime,identity));summary=runtime;write_rows(fullfile(out,'summary.csv'),stamp_rows(summary,identity));
    write_rows(fullfile(out,'source_identity_manifest.csv'),identity_source_rows(root,identity));
    write_rows(fullfile(out,'runtime_environment_manifest.csv'),identity_environment_rows(identity));
    save(fullfile(out,'stage4a7_3_summary.mat'),'summary','sc','domain_model','method_rows','metrics','non_metrics','non_audit','identity','-v7');
...
function rows=select_domain_method(dev,cal,sc)
    ...
    for k=1:numel(methods)
        ...
        rows(k).method_id=methods{k};rows(k).in_domain_acceptance=mean([z(in).domain_accepted]);rows(k).medium_far_ood_false_acceptance=mean([z(mf).domain_accepted]);rows(k).threshold=m.threshold;rows(k).near_boundary_threshold=m.near_boundary_threshold;rows(k).gate_pass=rows(k).in_domain_acceptance>=sc.parameter_domain.acceptance_floor;rows(k).domain_calibration_hash=m.calibration_hash;
    end
    pass=find([rows.gate_pass]);
    if isempty(pass),bestA=1;status='no_method_meets_gate';
    else
        bestA=pass(1);for k=pass(2:end),if rows(k).medium_far_ood_false_acceptance<rows(bestA).medium_far_ood_false_acceptance-1e-12,bestA=k;end,end
        status='deterministic_execution_fallback_no_statistical_comparison';
    end
    % Rule B is a predeclared sensitivity analysis only: strict lexicographic
    % maximization of in-domain acceptance, then minimization of OOD false
    % acceptance.  It never uses Pilot rows.
    bestB=1;for k=2:numel(rows)
        if rows(k).in_domain_acceptance>rows(bestB).in_domain_acceptance+1e-12 || ...
                (abs(rows(k).in_domain_acceptance-rows(bestB).in_domain_acceptance)<=1e-12 && rows(k).medium_far_ood_false_acceptance<rows(bestB).medium_far_ood_false_acceptance-1e-12),bestB=k;end
    end
    tied=find(abs([rows.medium_far_ood_false_acceptance]-rows(bestA).medium_far_ood_false_acceptance)<=1e-12 & [rows.gate_pass]==rows(bestA).gate_pass);
    for k=1:numel(rows)
        rows(k).selected_for_execution=k==bestA;rows(k).scientifically_unique_winner=false;rows(k).selection_status=status;rows(k).tied_methods=strjoin({rows(tied).method_id},',');
        rows(k).selection_rule_id=sc.parameter_domain.selection_rule_id;rows(k).selection_rule_version=sc.parameter_domain.selection_rule_version;rows(k).acceptance_floor=sc.parameter_domain.acceptance_floor;rows(k).eligible_method_count=numel(pass);rows(k).primary_objective=sc.parameter_domain.primary_objective;rows(k).tie_break_rule=sc.parameter_domain.tie_break_rule;rows(k).selection_split=sc.parameter_domain.selection_split;rows(k).pilot_used_for_selection=false;rows(k).canonical_rule_a_selected_method=methods{bestA};rows(k).strict_lexicographic_rule_b_selected_method=methods{bestB};
    end
end
...
function q=rate_metric(category,name,success,cid,seed,eh,ch)
    r=stage4a7_3_cluster_rate(success,cid,struct('replicates',1000,'seed',seed,'alpha',.05,'resampling_unit','candidate_id_cluster'));q=metric_row();q.category=category;q.metric_id=name;q.numerator=r.numerator;q.denominator=r.denominator;q.rate=r.rate;q.ci_low=r.ci_low;q.ci_high=r.ci_high;q.cluster_count=r.cluster_count;q.resampling_unit=r.resampling_unit;q.bootstrap_replicates=r.bootstrap_replicates;q.bootstrap_seed=r.bootstrap_seed;q.ci_method=r.ci_method;q.experiment_hash=eh;q.domain_calibration_hash=ch;
end
function [rows,metrics,audit]=nonunique_control(base,sc,eh)
    ...
    levels=unique([rows.noise_snr_db],'stable');metrics=repmat(non_metric(),0,1);for q=1:numel(levels),x=rows([rows.noise_snr_db]==levels(q));r=stage4a7_3_cluster_rate([x.false_unique],{x.sample_id},struct('replicates',1000,'seed',20263200+q,'resampling_unit','test_sample'));m=non_metric();m.noise_snr_db=levels(q);m.false_unique_numerator=r.numerator;m.false_unique_denominator=r.denominator;m.false_unique_rate=r.rate;m.ci_low=r.ci_low;m.ci_high=r.ci_high;m.complete_equivalence_coverage=mean([x.contains_complete]);m.singleton_rate=mean([x.set_size]==1);m.empty_set_rate=mean([x.set_size]==0);m.calibration_hash=model.calibration_hash;m.ci_method=r.ci_method;m.resampling_unit=r.resampling_unit;m.bootstrap_replicates=r.bootstrap_replicates;m.bootstrap_seed=r.bootstrap_seed;m.cluster_count=r.cluster_count;metrics(end+1)=m;end
end
...
function r=method_row(),r=struct('method_id','','in_domain_acceptance',NaN,'medium_far_ood_false_acceptance',NaN,'threshold',NaN,'near_boundary_threshold',NaN,'gate_pass',false,'selected_for_execution',false,'scientifically_unique_winner',false,'selection_status','','tied_methods','','domain_calibration_hash','','selection_rule_id','','selection_rule_version','','acceptance_floor',NaN,'eligible_method_count',0,'primary_objective','','tie_break_rule','','selection_split','','pilot_used_for_selection',false,'canonical_rule_a_selected_method','','strict_lexicographic_rule_b_selected_method','');end
function r=metric_row(),r=struct('category','','metric_id','','numerator',0,'denominator',0,'rate',NaN,'ci_low',NaN,'ci_high',NaN,'cluster_count',0,'ci_method','','resampling_unit','','bootstrap_replicates',0,'bootstrap_seed',0,'experiment_hash','','domain_calibration_hash','');end
...
function r=non_metric(),r=struct('noise_snr_db',NaN,'false_unique_numerator',0,'false_unique_denominator',0,'false_unique_rate',NaN,'ci_low',NaN,'ci_high',NaN,'complete_equivalence_coverage',NaN,'singleton_rate',NaN,'empty_set_rate',NaN,'calibration_hash','','ci_method','','resampling_unit','','bootstrap_replicates',0,'bootstrap_seed',0,'cluster_count',0);end
...
function rows=selection_sensitivity(method_rows,pilot,sc)
    cats=unique({pilot.category},'stable');rows=repmat(struct('rule_id','','selected_method','','category','','sample_count',0,'accepted_count',0,'acceptance_rate',NaN,'rejection_rate',NaN,'threshold',NaN,'pilot_used_for_selection',false,'scientifically_unique_winner',false),0,1);
    selected={method_rows(1).canonical_rule_a_selected_method,method_rows(1).strict_lexicographic_rule_b_selected_method};rules={'rule_A_gate_then_ood_canonical','rule_B_strict_lexicographic_sensitivity'};
    for q=1:2
        ix=find(strcmp({method_rows.method_id},selected{q}),1);threshold=method_rows(ix).threshold;method=selected{q};
        for k=1:numel(cats)
            x=pilot(strcmp({pilot.category},cats{k}));score=score_vector(x,method);ok=isfinite(score)&score<=threshold;r=rows_template_sensitivity();r.rule_id=rules{q};r.selected_method=method;r.category=cats{k};r.sample_count=numel(x);r.accepted_count=nnz(ok);r.acceptance_rate=mean(ok);r.rejection_rate=1-r.acceptance_rate;r.threshold=threshold;rows(end+1)=r; %#ok<AGROW>
        end
    end
end
...
function v=getenv_default(name,default),v=getenv(name);if isempty(v),v=default;end,end
