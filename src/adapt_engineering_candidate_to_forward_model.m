...
%   represented is returned with a reason code and is never silently scored.
    ...
    report.adapter_hash=stage4a4_scientific_config_hash(struct('adapter_schema','stage4a7_2_adapter_metadata_v2', ...
        'topology_key',getf(candidate,'topology_key',getf(candidate,'canonical_graph_key','')), ...
        'asset_state_key',getf(candidate,'asset_state_key',''), ...
        'source_node_id',source_id,'receiver_node_id',receiver_id,'network',net, ...
        'cfg',safe_cfg(cfg))); candidate.adapter_hash=report.adapter_hash;
