...
%STAGE4A7_3_CLUSTER_RATE Percentile bootstrap interval for a rate.
    ...
    clusters=unique(cluster_ids,'stable');nc=numel(clusters);B=getf(opts,'replicates',2000);seed=getf(opts,'seed',20263161);alpha=getf(opts,'alpha',.05);unit=getf(opts,'resampling_unit','candidate_id_cluster');
    ...
    r=struct('numerator',nnz(success),'denominator',numel(success),'rate',mean(success), ...
        'ci_low',lo,'ci_high',hi,'cluster_count',nc,'resampling_unit',unit, ...
