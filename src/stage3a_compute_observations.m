...
%   derived proxies and are not time-domain FDR/TFDR measurements.
    ...
    if info.is_counterfactual
        [measurements,meta]=plc_measurement_bundle(info.kind,network,theta,cfg_theta);
        [views,solve]=plc_counterfactual_multiview_response(f_hz,network,measurements,cfg_theta);
    ...
    elseif strcmp(info.O,'ordinary_ofdm_cfr')
        [measurements,meta]=plc_measurement_bundle(info.kind,network,theta,cfg_theta);
        ...
        details=struct('O',info.O,'physical_quantity',info.physical_quantity, ...
            ...
            'input_impedance_ohm',Zin,'is_proxy',true,'is_counterfactual',false, ...
